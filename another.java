    ...
    private static void loadBooksFromFile(Library library) {
        try (BufferedReader reader = new BufferedReader(new FileReader(FILE_PATH))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                String edition = bookInfo[3];
                int quantity = Integer.parseInt(bookInfo[4]);
    ...
    private static void saveBooksToFile(Library library) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(FILE_PATH))) {
            for (Book book : library.books) {
                writer.write(String.format("%s,%s,%s,%s,%d%n", book.getTitle(), book.getAuthor(), book.getPublisher(), book.getEdition(), book.getQuantity()));
            }
        } catch (IOException e) {
